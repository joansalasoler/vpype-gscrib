...
class GRenderer(DocumentRenderer):
    ...
    def _write_document_header(self, document: Document):
        ...
        self._g.comment(self.HEAD_SEPARATOR)
    ...
    def _write_layer_header(self, layer: LineCollection):
        ...
        last_ctx = self._previous_context()

        self._g.comment(self.SECTION_SEPARATOR)
        self._g.comment(f'Layer: {layer_name}')
        self._write_layer_config_info(self._context)
        self._g.comment(self.SECTION_SEPARATOR)
    ...
    def _write_layer_config_info(self, context: GContext):
        """Write configuration changes as G-code comments."""

        document_ctx = self._document_context
        document_values = document_ctx.format_config_values()
        layer_values = context.format_config_values()

        changed_settings = {
            key: value
            for key, value in layer_values.items()
            if (key, value) not in document_values.items()
        }
