...
With the option `direct_write_mode`, a serial connection to the controlled device
...
g = mecode.G(
    ...
    baudrate=115200
)  # Under MS Windows, use port="COMx" where x has to be replaced by the port number of the virtual COM port the device is connected to according to the device manager.
