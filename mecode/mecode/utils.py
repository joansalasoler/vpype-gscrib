...
def profile_surface(
    g, kp, x_start, x_stop, x_step, y_start, y_stop, y_step, feed_rate=5, dwell=0.1
):
    """
    Parameters
    ----------
...
def write_cal_file(
    ...
    with open(path, mode) as f:
        ...
        f.write(
            ":START2D    2          1           1            2           {}          -{}        {}\n".format(
                y_step, x_step, num_cols
            )
        )  # noqa
        f.write(
            ":START2D OUTAXIS3={} POSUNIT=PRIMARY CORUNIT=PRIMARY OFFSETROW = {} OFFSETCOL={}\n".format(
                axis, -(y_start + y_offset), -(x_start + x_offset)
            )
        )  # noqa
